...
| Per-lane API key | OPT-OUT | not needed yet — target shape is a local embedder (ignores `Authorization`) plus hosted chat, so the shared key works. Deferred in `26-CONTEXT.md`; required once both lanes are hosted separately. |
...
| Tool / function calling | OPT-OUT | explicitly out of scope — the summarizer is a single-shot compressor over fenced untrusted content; tool-call capability would widen the prompt-injection surface for no benefit. |
| Structured outputs / JSON mode | OPT-OUT | not needed yet — the summary contract is one plain line capped at `ENGRAM_SUMMARY_MAX_CHARS`; a schema-constrained response would add provider-compatibility risk across supported gateways. |
| `GET /v1/models` (model discovery) | OPT-OUT | not needed — model ids are operator-configured (`ENGRAM_EMBED_MODEL`, `ENGRAM_SUMMARY_MODEL`) and payload-stamped for reindex provenance; discovery would make that identity non-deterministic. |
...
| Reranking endpoints (provider-side rerank) | OPT-OUT | explicitly out of scope — reranking is in-process and dependency-free (`store.SearchReranked`, the stdlib lexical reranker from v0.9.x); an outbound call would egress query text on every recall. |
