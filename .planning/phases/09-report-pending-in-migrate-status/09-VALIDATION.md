...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 09-01-T1 | 09-01 | 1 | REQ-migrate-status-histogram | — | N/A | unit | `go test ./internal/store/... -run 'TestMigrateStatusResultPending$\|TestMigrateStatusResultPendingZeroValue' -v` (pre-existing; proves the source-of-truth method, unaffected by this phase — reference only, not re-verified) | ✅ `internal/store/migrate_status_test.go:29,49` | ✅ green |
| 09-01-T1 | 09-01 | 1 | REQ-migrate-status-histogram | — | N/A | unit | `go test ./cmd/engram/... -run TestMigrateFamilyStatusReportDocPendingNeverRederived -v` — five subtests: `equals_store_pending`, `fixture_discriminates_every_naive_rederivation`, `zero_value_marshals_pending_zero`, `converter_is_pure`, `renders_last_in_both_lanes` | ✅ `cmd/engram/migrate_family_test.go:453`  ✅ green |
| 09-01-T1 | 09-01 | 1 | REQ-migrate-status-histogram | — | N/A | unit | `go test ./cmd/engram/... -run TestMigrateFamilyStatusReportDocKeyOrder -v` (existing; `want` extended with `"pending"` last, D-06) | ✅ `cmd/engram/migrate_family_test.go:420` | ✅ green |
| 09-01-T1 | 09-01 | 1 | REQ-migrate-status-histogram | — | N/A | unit | `go test ./cmd/engram/... -run TestMigrateFamilyStatusReportDocNeverMarshalsNull -v` (existing, unchanged — regression guard) | ✅ `cmd/engram/migrate_family_test.go:409` | ✅ green |
| 09-01-T1 | 09-01 | 1 | REQ-migrate-status-histogram | — | N/A | unit | `go test ./cmd/engram/... -run 'TestOperatorViewIdentityAcrossEveryOperatorCommand\|TestMigrateViewIdentity' -v` (existing; picks up the new field via the shared fixture, D-08) | ✅ | ✅ green |
| 09-01-T1 | 09-01 | 1 | REQ-migrate-status-histogram | T-09-01 | Operator report stays hand-declared, so record content is unreachable by construction | unit | `go test ./cmd/engram/... -run TestOperatorDocsAreHandDeclared -v` (existing; a `uint64` scalar does not trip the embedding check) | ✅ | ✅ green |
| 09-01-T2 | 09-01 | 1 | REQ-migrate-status-histogram | — | N/A | unit | `go test ./cmd/engram/... -run TestMigrateFamilyStatusSummaryPendingClause -v` — three subtests: `ordering_tokens_are_unambiguous`, `states_pending_value_between_buckets_and_future`, `emitted_unconditionally_at_zero` (D-03) | ✅ `cmd/engram/migrate_family_test.go:587`  ✅ green |
| 09-02-T1 | 09-02 | 2 | REQ-docs-record-state | T-09-05 | The gate that closes a vacuous-gate finding is itself proven discriminating | doc gate (Go test over file content) | `go test ./cmd/engram/... -run TestMigrateGuidePendingRow -v` — expected NON-zero exit **at that task** (RED observed before the fix landed; green now): `TestMigrateGuidePendingRowIsAccurate` RED against the live guide while `TestMigrateGuidePendingRowGateFiresOnInjectedViolation` passes all seven cases | ✅ `cmd/engram/migrate_docs_test.go:101,124`  ✅ green |
| 09-02-T2 | 09-02 | 2 | REQ-docs-record-state | T-09-06 | Corrected row names the `future` exclusion an operator is most likely to get wrong | doc gate (Go test over file content) | `go test ./cmd/engram/... -run TestMigrateGuidePendingRow -v` (now green) plus the scoped zero-occurrence sweep `rg -o -F <anchor> docs-site/ \| wc -l` returning 0 for each of the two anchors | ✅ `cmd/engram/migrate_docs_test.go:101` + scoped `rg` sweep  ✅ green |
...
- [x] **Owner: 09-01-T1** — `TestMigrateFamilyStatusReportDocPendingNeverRederived`. New discriminating unit test for D-05, co-located with `TestMigrateFamilyStatusReportDocKeyOrder`
      ...
      would pass it.
- [x] **Owner: 09-01-T2** — `TestMigrateFamilyStatusSummaryPendingClause`. New unit test for D-03,
...
- [x] **Owner: 09-02-T1** — `cmd/engram/migrate_docs_test.go`. New W3 doc-content gate for D-07.
...
- [x] **Owner: 09-02-T1** — `TestMigrateGuidePendingRowGateFiresOnInjectedViolation`, the positive
...
- [x] Wave 0 covers all MISSING references — all four Wave 0 items exist and are green
- [x] No watch-mode flags
- [x] Feedback latency < 5s — quick run measured at **1.111s** on 2026-08-22 (exit 0, 53 `--- PASS`, 0 `--- FAIL`)
...
Audited after phase execution. Every row in the Per-Task Verification Map was
re-run against the merged tree rather than trusted from the plan-time seeding.
All 9 top-level tests pass and every named subtest is present and green:

- `TestMigrateFamilyStatusReportDocPendingNeverRederived` — all 5 subtests
  (`equals_store_pending`, `fixture_discriminates_every_naive_rederivation`,
  `zero_value_marshals_pending_zero`, `converter_is_pure`,
  `renders_last_in_both_lanes`)
- `TestMigrateFamilyStatusSummaryPendingClause` — all 3 subtests
  (`ordering_tokens_are_unambiguous`,
  `states_pending_value_between_buckets_and_future`,
  `emitted_unconditionally_at_zero`)
- `TestMigrateGuidePendingRowGateFiresOnInjectedViolation` — all 7 control cases,
  including the load-bearing `clean` case that proves the gate discriminates
  rather than firing unconditionally

The 09-02-T2 zero-occurrence sweep returns 0 for both stale-claim anchors, with a
non-zero control (`pending` → 5 hits in the same file) confirming the search
reaches the guide rather than returning empty for an unrelated reason.

No verification in this phase is manual-only. `status` advances `draft` →
`validated`; `nyquist_compliant` is now authoritative and `true`.
