...
| Claude Code / opencode tolerate the new `metadata.engram-summary` frontmatter key without warning or dropping the skill | REQ-skills-native-format | Same ownership-boundary rule — this is third-party loader behavior, verifiable but not gateable. | Install skills, start each runtime, confirm all five skills load and no frontmatter warning is emitted. **Verified 2026-09-10 (Sean), codex only:** the five skills loaded and were listed by codex; the unrecognized `metadata.engram-summary` key did not cause codex to reject or drop them (warning output not separately observed). Claude Code and opencode were NOT observed this round — still pending manual checks. |
