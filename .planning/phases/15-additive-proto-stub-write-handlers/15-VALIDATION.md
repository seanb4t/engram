...
| 15-01-03 | 01 | 1 | REQ-connect-write-rpcs | T-15-SC | Codegen idempotent (index-safe snapshot diff); option-types dep → direct, runtime stays indirect | build + drift | `go tool buf generate && go build ./... && D=$(mktemp -d) && cp -R gen "$D/gen" && go tool buf generate && diff -r gen "$D/gen"; RC=$?; rm -rf "$D"; test $RC -eq 0` | ✅ `gen/` (regen) | ⬜ pending |
...
| 15-03-01 | 03 | 2 | REQ-connect-write-rpcs | T-15-03 | Malformed payloads → `CodeInvalidArgument` (real validator, proves annotations load); non-validation error → `CodeInternal` (fake validator); runtime dep promoted to direct | unit (TDD) | `go test ./internal/server/... -run TestConnectValidateInterceptor -v` | ❌ W0 (new files) | ⬜ pending |
...
| 15-04-01 | 04 | 3 | REQ-connect-write-rpcs | T-15-01 | 11 RPCs all `IDEMPOTENCY_UNKNOWN`; read-lane per-field table (number/name/kind/cardinality/type) pinned; six write RPC req/resp types pinned | descriptor unit | `go test ./internal/server/... -run TestEngramServiceDescriptor -v` | ❌ W0 (new file) | ⬜ pending |
| 15-04-02 | 04 | 3 | REQ-connect-write-rpcs | T-15-01 / T-15-02 | Exact codes per cell: Unimplemented / Unauthenticated / GET-405 (via generated Procedure constants) / InvalidArgument ×6 RPCs; plus D-03 mask cells (absent/empty/unknown → InvalidArgument) + category-allowlist cells | unit (httptest matrix) | `go test ./internal/server/... -run TestWriteRPCNegativeMatrix -v` | ❌ W0 (new file) | ⬜ pending |
