...
1. **Exact eval package location: `internal/retrievaleval/` (new) vs. extending `internal/store`'s existing integration suite? (RESOLVED — 09-01-PLAN: new `package retrievaleval` importing `internal/server`/`store`/`embed`, routed through the shared `store.SearchReranked` helper per Codex review finding 5)**
...
2. **What `k` and dataset size are needed for a statistically meaningful recall@k/MRR on a single #261 fixture? (RESOLVED — 09-01-PLAN: #261 fixture mandatory with ≥ defaultK+1 (12–20) sticky distractors per Codex review finding 4; broader synthetic set kept optional/stretch)**
