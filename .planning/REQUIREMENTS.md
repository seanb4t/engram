...
- [x] **REQ-cask-reship-recovery**: A failure between tag creation and cask publication is recoverable without hand-editing the tap, by extending this repo's existing `workflow_dispatch` re-ship path rather than inventing a second mechanism. The recovery is rehearsed once, not assumed. **Acceptance clarification:** Phase 1 Context D-15 supersedes the literal rehearsal criterion with D-14 guard and D-13 credential-probe verification; no staged rehearsal is performed. The Phase 1 verification report accepts this by construction, and the credential probe passed from main (run 32860661930).
...
- [x] **REQ-register-claude-code**: `engram setup` registers the engram MCP server with Claude Code by invoking `claude mcp add`, never by hand-writing `~/.claude.json` or `.mcp.json`. This preserves what the shipped `/engram-setup` prose already does correctly.
