...
- **D-11 (plan-time, user-confirmed 2026-09-19):** Payload fields that still have no write cap after D-01/D-10 (`scope`/`repo`/`workspace`/`worktree_path`/`base_dir`/`source`; citation `ref`/`locator`/`pin`; `supersedes` target count; discovery/rule `tags`) get a fixed **16 KiB allowance** in the per-record ceiling (03-02) — NOT new write caps this phase. Over-allowance records take the D-07 batch-of-1 fallback → named `ErrResponseTooLarge`, never a silent skip. Capping them is tracked as GitHub #589 (each a separate one-way write contract). D-10's "fully provable" wording is therefore qualified: provable for the capped fields, allowance-bounded for the rest.
