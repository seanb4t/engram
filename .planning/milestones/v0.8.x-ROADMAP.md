# Milestone v0.8.x: Baseline + Connect Auth Hardening

**Status:** ✅ SHIPPED
**Phases:** 1–8
**Total Plans:** N/A (retrospective — shipped before GSD planning)

## Overview

Retrospective / as-built record of the v0.8.x baseline. Phases 1–7 group the already-completed
v0.8.x work by synthesis area; Phase 8 (Connect observe-lane auth hardening, R1–R4) was **found
already shipped** during a 2026-07-08 reconciliation — the cookie/OIDC lane landed opportunistically
inside PR #248 and was hardened in PR #266.

All 56 ADR-locked decisions (25 core + 31 companion refinements, folded 2026-07-08) and all 24
routed v0.8.x requirements are implemented and merged to main. Per-phase implementation plans are
cross-referenced in `.planning/intel/merge-plans/context.md`.

**Archived from `.planning/ROADMAP.md` on 2026-07-31.** These sections were moved out because GSD's
phase resolvers match a bare `### Phase N:` heading, and v0.12.x restarted phase numbering at 1 —
so these historical headings were shadowing the active milestone's phases 1–6. Archiving them here
matches how v0.9.x–v0.11.x were already handled and restores single-milestone resolution in
`ROADMAP.md`. See the phase-numbering note in `ROADMAP.md`'s Progress section.
