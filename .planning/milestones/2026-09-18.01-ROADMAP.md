...
- [x] 04-02-PLAN.md — one exported recall maximum, `Store.List`'s cursor and offset modes composed from the ordered-page primitive, and Phase 2's overflow regressions retargeted (D-01, D-02, D-05, D-06, D-12)
...
- [x] 04-04-PLAN.md — two-phase search: a payload-free query plus a batched id-set payload fetch that re-applies the caller's filter (D-09)

**Wave 5**

- [x] 04-05-PLAN.md — per-caller payload projection with a no-summary content backfill, and the store's refuse-never-clamp backstop (Phase 3 D-04, D-10)
...
- [x] 07-03-PLAN.md — Summarize lane: named error-body bound, both drain sites, both regression shapes, and the summarize timeout ceiling
- [x] 07-04-PLAN.md — Config-to-client wiring with a source gate, plus the eight documented variables and the upgrade note
- [x] 07-05-PLAN.md — Five red-evidence patches registered, both requirements ticked, GitHub #347 closed on cited evidence
