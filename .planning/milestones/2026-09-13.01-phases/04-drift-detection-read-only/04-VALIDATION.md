...
- [x] `internal/setup/drift_test.go` (or equivalent) — shared `Facet`/comparison pure-function tests (REQ-drift-three-way, REQ-drift-facet-naming)
- [x] `internal/setup/claudecode_test.go` extension — Claude Code scan fixtures; the literal-echo case is gated on `04-OBSERVATIONS.md` (D-06)
- [x] `internal/setup/codex_test.go` extension — Codex scan fixtures, same gate
- [x] `internal/setup/plan_test.go`-adjacent — `TestRedactionUnconditional` (REQ-drift-redaction), the read-path mirror of `TestNoSecretInArgs`
- [x] `cmd/engram/setup_test.go` / `operator_view_setup_test.go` extension — facet rendering, `setupApplySummary` preserved-count, `--output json` leak test
- [x] `cmd/engram/migrate_docs_test.go`-style docs gate for `guides/agent-setup.md`'s new `preserved` row and opencode not-compared statement
- [x] `.planning/phases/04-drift-detection-read-only/04-OBSERVATIONS.md` — NOT a Go test; the D-05/D-08 human-checkpoint deliverable that scanner fixtures depend on
...
Retroactive reconciliation by `/gsd-validate-phase 4` at `main` HEAD `fbcf587f` (post-merge of PR #569): every
...
repeating it. Wave 0 files exist and are the tests the rows name. Row 04-02-01 is the
manual D-05 observation protocol (rule m45p2b4bp7); it was completed by the maintainer on 2026-09-15 and is
recorded in `04-OBSERVATIONS.md`, so it is marked green as an observed, not automated, verification. Row
04-05-02's gate was repaired in place: the plan-authored `test "$(rg -c -F … )" -eq 0` can never pass on the
intended zero-match case because `rg -c` prints nothing (exit 1) when there are no matches; it now counts with
`rg -o … | wc -l` (gotcha v2rbxwg2r8 family). The assertion's intent — zero `ASSUMED SHAPE (A3)` markers left in
`codex_test.go` — holds.
