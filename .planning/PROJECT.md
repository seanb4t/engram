...
v0.12.x closed 2026-08-02. Candidate scope parked in `ROADMAP.md`'s Backlog:

- **Phase 999.1** — a spine review / consolidate / verify / purge / archive command: one operation
  that audits a memory spine end to end.
- **Phase 999.2** — review the whole CLI and MCP surface under the correct-by-reading principle
  (`4aksmneehh`), the standard Phase 7 raised to a milestone-wide bar.

Promote with `/gsd-review-backlog`, or scope something new with `/gsd-new-milestone`.
...
- **Nyquist validation reconciliation** — no v0.12.x phase has a reconciled `VALIDATION.md`.
- **Two-tier CLI error model** — the client-side usage-error/exit-2 lane has no REQ or decision ID
  recording its exemption from the field+hint envelope.
