...
- **Gate & CI integrity** — key-link `pattern:` fields with `\\` escaping are silently unmatchable,
  so v0.13.x Phases 1–2's key-link gates were no-ops (#479); `internal/store`'s Qdrant
  testcontainer dies mid-run and masks genuine failures (#497). Both land first, because this
  milestone authors new key-links and must be able to trust a red build.
