...
- **Connect record-state parity** — `proto`'s `Memory` gains `superseded_by`, `supersedes`,
  `not_before`, `not_after`, `archived_at` plus `schema_version` in ONE additive pass, wired through
  `memoryToProto` (#482). Ordered after the schema work because proto field numbers are a permanent
  one-way commitment.
