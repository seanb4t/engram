...
## Current State: v0.13.x — Curation & Self-Evidence ✅ SHIPPED (2026-08-12)
...
## Current Milestone: 2026-08-12.01 Record State & Schema Evolution

**Goal:** a record's full state — supersession, scheduling, archival, and its own schema version —
is reachable and legible on every lane, and payload evolution has a real mechanism instead of
another one-shot operator command.

**Target features:**

- **Gate & CI integrity** ✅ **Phase 1 complete (2026-08-13)** — key-link `pattern:` fields with `\\`
  escaping are silently unmatchable, so v0.13.x Phases 1–2's key-link gates were no-ops (#479);
  `internal/store`'s Qdrant testcontainer dies mid-run and masks genuine failures (#497). Both land
  first, because this milestone authors new key-links and must be able to trust a red build.
  Delivered: `internal/keylinks` plus two recurring gates with asymmetric scopes (escaping
  time-invariant → repo-wide; satisfiability HEAD-dependent → active milestone only), 39 escaped
  patterns normalized across 20 plan files, all 30 v0.13.x Phase 1–2 links reassessed (26 pinned,
  3 pinned-via-target, 1 unpinned, recorded not repaired); CI collapsed to one shared `services:`
  Qdrant with structural per-package collection prefixes and a go/ast conformance gate.
  Two caveats carried forward: the upstream gsd-core parser gap is spine-tracked, not filed
  (`h7ac73v2r8`), so `internal/keylinks` is a **permanent** guard; and the shared instance covers
  **three** packages, not four — `internal/retrievaleval` never dials Qdrant in CI.
- **Record schema versioning** — a `schema_version` discriminator on the record payload. Absent
  means v0, so adoption needs no backfill. Mirrors the shipped `internal/webauth`
  `sessionPayloadVersion` pattern (auto-inject on write, explicit check on read).
- **Migration mechanism** — a versioned `engram migrate`, preview-by-default via the v0.13.x
  `registerDestructive` tier, subsuming `backfill-short-ids`. Deliberately does **not** swallow the
  operational sweeps that are not version-driven: `migrate-remap-owner` (IdP claim re-stamp),
  `summarize-missing` (ongoing async-fill sweep), `reindex` (embedder config identity).
- **Connect record-state parity** ✅ **Phase 5 complete (2026-08-15)** — `proto`'s `Memory` gains
  `superseded_by`, `supersedes`, `not_before`, `not_after`, `archived_at` plus `schema_version` in
  ONE additive pass, wired through `memoryToProto` (#482). Ordered after the schema work because
  proto field numbers are a permanent one-way commitment.
  Delivered: EIGHT fields at numbers 23–30 (`summary_model` and `summary_egress_at` joined the six
  originally scoped), under D-14's non-uniform presence models — `schema_version`/`summary_model`
  assigned unconditionally because protojson omits an unset `optional` even with
  `EmitDefaultValues`, while `superseded_by` stays a direct pointer copy that remains unset on nil.
  The parity proof is a reflection detector deriving its inclusion rule from `json:"-"`, with a
  permanent negative fixture proving it can reject — `buf breaking` is structurally blind to
  `memoryToProto`'s Go body and could never have caught the gap it missed three milestones running.
  One caveat carried forward: SC3 asked for the no-op-rounding rationale to live in the
  `not_before`/`not_after` **proto field comments**, but `internal/surfaces`' `checkProtoSurface`
  matches proto comments by bare field name across every message, so a comment there binds the
  `schedule-window-*` rules anchored on `ScheduleMemoryRequest`. The rationale lives in
  `memoryToProto` instead (`44366849`); accepted as a location change by the phase verifier, and
  SC3 was amended to name the Go mapper (`416bafab`) so the criterion and the tree now agree.
- **Typed operator renderer** — make json/text widening structurally unrepresentable rather than
  test-detected, ahead of six new fields flowing through `renderOperator` (#481).
- **Console + CLI surfacing** — show archived / superseded / scheduled and migration state; the
  operator console cannot render the v0.13.x archive tier at all today.
- **Registry + docs tail** — register `RuleSweepScopeOrAllScopesRequired` for the
  `--scope`-or-`--all-scopes` guard shared by `summarize-missing` and `spine-review scan` (#480);
  update `reference/memory-record.md` and `reference/tools.md`, and revise CLAUDE.md's
  "Not used here: database migrations" convention, which this milestone supersedes.

**Key context:** first milestone under the CalVer label convention (rule `e325awbf7x`). v0.13.0 is
released but **not deployed** — three milestones of code have still never run outside tests and
testcontainers, so first-deploy surprises remain an open risk. Standing constraint: zero new Go
dependencies.

**Deferred out of this milestone:**

- **`REQ-consent-adversarial-proof`** — the consent gate is still unproven against a confidently
  *wrong* proposal. The 3-run cap produced only correct verdicts, so the adversarial moment never
  arrived. Reaching it needs a fixture that reliably misleads on identity, not more runs of the
  same one (`WINDOWS.md` id 3).
- **Cross-spine `ListScopes` failure discarding hits** (#456) — filed explicitly as *not* a defect;
  the behavior is deliberate and documented. Revisit only if it shows up in practice.
- **Full-stack console e2e harness** (#366) — large, and overlaps #497's infrastructure story.

**Deliberately still open (not yet scoped):**
...
_The v0.12.x two-tier CLI error model gap is closed — v0.13.x Phase 1 unified the taxonomy rather
than documenting a boundary, which is what #467 asked for._
...
Milestone `2026-08-12.01` — Record State & Schema Evolution — is open (see **Current Milestone**
above). REQ-IDs are being defined; `.planning/REQUIREMENTS.md` and `.planning/ROADMAP.md` carry the
scoped set and its phase mapping once written.
...
- [ ] **Versioned payload-migration mechanism** (v0.13.x) — no stored schema/payload version exists anywhere in `internal/store/` or `cmd/engram/`; every payload evolution has shipped as its own one-shot operator command with a near-identical scroll → `SetPayload` → `--dry-run` → `--timeout` shape. Research a general mechanism before the next one.
...
*Last updated: 2026-08-15 — Phase 5 (Connect Record-State Parity) COMPLETE: 3/3 plans, verification 3/3, code review 0 Critical / 0 Warning / 1 Info. Eight fields at proto numbers 23–30 under D-14's deliberately non-uniform presence models. The parity proof is a reflection detector keyed on `json:"-"` with a permanent negative fixture proving it can reject — the point being that `buf breaking` operates on the schema and has zero visibility into `memoryToProto`'s Go body, which is why it stayed green through the same gap in v0.8.x, v0.11.x and v0.13.x. Two shipped gates went red on the wave-1 merge, both verified green at base `059807ab` first, so both were genuine regressions from this phase: `internal/surfaces` (a comment on the new `Memory.not_before` bound the `schedule-window-*` rules anchored on `ScheduleMemoryRequest`, because `checkProtoSurface` matches proto comments by bare field name across every message — rationale relocated to `memoryToProto`, `44366849`, recorded as `3x25etde4f`) and `internal/keylinks` (all three phase-05 plans authored `key_links:` as bare prose instead of the `from`/`to`/`via`/`pattern` mapping; invisible until a plan gets a SUMMARY, so it detonated on success with an empty PLAN.md diff — converted in `ae483201`, recorded as `chwrfp8q0v`). The verifier accepted the Go-mapper location as an override rather than a gap and recommended amending SC3; done in `416bafab`, which now names `memoryToProto` and records why the rationale must not be moved back onto the proto field comments. `05-VERIFICATION.md` still describes SC3 in its pre-amendment wording — that is the accurate record of what the verifier evaluated and is deliberately left as written. Previously: 2026-08-13 — Phase 1 (Gate & CI Integrity) COMPLETE: 6/6 plans, verification 3/3, CI green on run 31720625349. Code review found two Criticals the phase verifier missed — both instances of the vacuous-gate defect the phase itself exists to eliminate, reproduced in its own gates — fixed in `940db28c` (zero-applicability guard on both key-link gates) and `efe68925` (AST conformance scan anchored on the enclosing function rather than the argument's syntactic shape). Methodology note recorded as `x6v6qxqd6f`: goal-backward verification and adversarial review fail in different directions; run both. Previously: 2026-08-12 — OPENED milestone `2026-08-12.01` — Record State & Schema Evolution (`/gsd-new-milestone --reset-phase-numbers`, per rule `rvmts69cz1`). FIRST milestone under the CalVer label convention adopted this same day (rule `e325awbf7x`, PR #496): labels are `YYYY-MM-DD.NN` off the milestone's LOCAL START date, stamped once and never rewritten to a ship date; forward-only, so v0.8.x–v0.13.x keep their SemVer-style labels; releases stay SemVer and remain release-please's, since Helm's `Chart.yaml` requires SemVer 2 and Go module tags require `vX.Y.Z` — so milestone → release is now a LOOKUP, never an inference. Label validated per the revised rule `0v4249kc9d` before any write: one worktree on `main`, no workstreams, no open release PR, no existing `2026-08-12.NN` label anywhere, so `.01` was free. Scope anchors on #482 — the Connect lane's `Memory` message has ALWAYS omitted `superseded_by`, `supersedes`, `not_before`, `not_after`, and now `archived_at`, so three milestones of shipped state (v0.8.x scheduling, v0.11.x supersession, v0.13.x archive tier) are invisible to BOTH consumers of that lane: the operator console and `engram search|list`. Confirmed at scoping that #482 is WIRE-ONLY — those payload keys already exist on `store.Memory` — so it creates no schema pressure by itself. The user brought schema versioning and migrations INTO scope over an initial recommendation to defer (the todo's own verdict was "pressure is real but not yet acute", and CLAUDE.md states "Not used here: database migrations"); the user overrode both, so this milestone ALSO revises that convention line rather than shipping a doc that contradicts the code. Ordering decided on a hard constraint, not preference: schema versioning and the migration mechanism land BEFORE #482, because proto field numbers are a permanent one-way commitment here (a `deprecated = true` field still occupies its number; v0.11.x's `categories = 8` needed a blocking checkpoint for exactly this) — freezing `schema_version` on the wire before the mechanism settles its semantics would make any correction unfixable. Two phases ordered FIRST on gate-integrity grounds: #479 (key-link `pattern:` fields with `\\` escaping are silently unmatchable, so v0.13.x Phases 1–2's key-link gates were NO-OPS and any "key-links passed" on them is not evidence) must be fixed before this milestone authors its own key-links; and #497 (filed this session — `internal/store`'s Qdrant testcontainer dies mid-run, observed 3× in ~2h including on a DOCS-ONLY PR, and it already masked one genuine failure during the v0.13.x ship) must stop making a red build unadjudicable. Scoping deliberately EXCLUDED #456 after reading it — the issue itself states the behavior is "Not a defect", deliberate and documented — and #366 (large, overlaps #497). Migration scope bounded to what is genuinely version-driven: subsumes `backfill-short-ids`, deliberately NOT `migrate-remap-owner` (IdP claim re-stamp), `summarize-missing` (ongoing async-fill sweep), or `reindex` (embedder config identity). Research-first selected. Carried risk unchanged: v0.13.0 is RELEASED but NOT DEPLOYED, so three milestones of code have still never run outside tests and testcontainers. Prior: 2026-08-12 after CLOSING milestone v0.13.x — Curation & Self-Evidence (`/gsd-complete-milestone`): 6 phases (1–5 plus inserted 03.1), 33 plans, 99 tasks, 23/24 requirements, audit `tech_debt` (6/6 cross-phase integration seams wired, 4/4 E2E flows traced, 0 blockers, Nyquist 5/6 COMPLIANT); archived to `milestones/v0.13.x-*`. Delivered the curation half — `engram spine-review` (scan/verify/consolidate/purge/archive/restore) as the SIXTH instance of the existing Subject-less operator tier rather than a new authz path, with a four-tier citation classifier over a resolved-path safety gate, near-duplicate ranking that reuses stored vectors via `NewQueryID` (no re-embedding, no clustering, no default threshold, no mutation on any path), an orthogonal `archived_at` soft-hidden state, and a `PurgeManifest` whose provenance is compiler-enforced through unexported fields instead of a runtime check — paired with the `curating-spine` skill carrying the semantic judgments a CLI cannot make, propose-never-perform, reusing `store_rule`'s consent block byte-identically with zero server-side code. And the self-evidence half: `internal/surfaces` declares each conditional rule ONCE and a conformance gate proves its canonical sentence present on every surface its fields resolve to, with applicability DERIVED from the rule's own fields rather than declared, and an unexported `declared` marker making an off-registry rule a compile error in any other package (verified with scratch repros outside the owning package, not by re-reading the commit's own tests). One exit-code taxonomy now governs the whole binary — #467 was resolved BEFORE #453 on purpose, because cobra's `MarkFlagsMutuallyExclusive` raises a plain `fmt.Errorf` that bypasses `cliError`/`ExitCode()`, so adopting #453 first would have reintroduced the exact undocumented split one command over. `supersede_memory` grew multi-target merges with a target-set-keyed `mergeFingerprint` checked before the already-superseded stage. Zero new Go dependencies across all six phases, as research predicted. Closed with one requirement deliberately UNMET: `REQ-consent-adversarial-proof`'s cold read exhausted its 3-run cap with every run landing NOT-TEMPTED — the reader's identity verdict was correct each time, so the confidently-wrong case the criterion must observe was never produced — and the terminal NOT-OBTAINED verdict was accepted as a non-result rather than converted into a pass, staying visibly unproven across five artifacts (`WINDOWS.md` id 3 open). Pre-archive reconciliation repaired four bookkeeping defects the milestone audit found, all of which made the milestone read SMALLER than it was: REQUIREMENTS.md's traceability table carried 22 rows against 24 body checkboxes (`REQ-destructive-preview-default` and `REQ-operator-output-flag` were delivered, verified, and simply missing from the table — the inverse of the orphan case audit-milestone checks for, now memory `1a854afsj4`), ROADMAP.md's Progress rows for Phases 2 and 4 read `Not started` for completed phases via the known gsd-core wrong-milestone write (`cvvrwjbsnz`; true values recovered from `git show`, never invented), Phase 03.1 had no Progress row at all, and the milestone summary line still claimed "18/18 requirements mapped, 0 shipped". Carried tech debt: three open broken windows (two Phase 03 TDD commit-granularity deviations, one the unmet SC-3), three waived Phase 03 human-verification items, a stale rationale comment at `internal/surfaces/toolclass.go:141-142` contradicting the shipped `idempotency_key` support (annotation value correct), and #476. No successor milestone scoped — run `/gsd-new-milestone`. Prior: 2026-08-12 after Phase 5 (Validation Debt Reconciliation) — validated REQ-nyquist-reconciled + REQ-citation-fixture-355. The phase was descoped mid-discussion by the user ("why are we auditing things, capturing things to files for CI later? Seems like a hell of a lot of over engineering") and shipped as records only: NO CI gate, NO `task` target, NO committed script, NO Go test reading `.planning/**`, no reconciliation report artifact, and `.planning/milestones/**` left immutable. Phases 01/02/03.1 flipped to `status: validated` / `nyquist_compliant: true` after all 23 `-run` pattern elements were re-resolved live against `go test -list '.*' ./...` at HEAD (1,047 names) rather than by exit status — the distinction being the whole point, since `go test -run` exits 0 when its pattern matches NOTHING. 03.1's REQ-merge-idempotency command named `TestSupersedeIdempotency`, a plan-time seed that never shipped; repointed to the two funcs that did, with the drift written down rather than silently corrected. Phase 04 deliberately left PARTIAL (`status: validated` + `nyquist_compliant: false`): `REQ-consent-adversarial-proof`'s cold read terminated at NOT-OBTAINED, the user accepted the non-result on 2026-08-11, and its row still reads `⬜ pending` — an unproven requirement stays visibly unproven. A blocking checkpoint (05-01 Task 3) surfaced that locked decision D-03 rested on a false premise: 04's no-new-server-code row was RED at HEAD because a doc-binding test (`internal/server/verbtabledocs_test.go`, `a2599027`) landed inside its open-ended `72a32c58..HEAD` range during phase 04's own review-fix pass. User ruled `narrow-and-record` — both ends pinned and `*_test.go` excluded, recorded as a fourth numbered correction; the row had already been re-anchored once and broke again identically, so recurrence was the signal that an unpinned reference point, not the sha, was the defect. #355 repaired as the plain docs fix it is: citation anchors now name SYMBOLS (`store.EmbedText`, and `server.Register`'s search_memory closure) instead of `tools.go:NNN` line numbers, since a fresh line number re-arms the same trap on the next refactor. Code review found 1 Critical: the `defaultK` comment attributed the `a.K = 8` default to `deps.searchMemory`, whose own doc comment states it applies NO internal k default — the real MCP default is in the `Register` closure, and the only literal `a.K = 8` belongs to `deps.searchDiscovery`, a different tool. Root cause was upstream of the executor: `05-02-PLAN.md`'s own action text asserted both "sits at tools.go:1617" AND "inside deps.searchMemory" — two different functions. Fixed `98f92667`; the lesson (plans are audited far less than code) is in memory `28j4f4wq1f`. Verification also caught this phase's own GSD tracking write corrupting the SHIPPED v0.12.x Phase 5 Progress row — the known gsd-core `rowMatch` defect (`dffmk92a8q`), now extended by `cvvrwjbsnz` with the finding that it is NOT confined to `phase.complete` (the per-plan `roadmap.update-plan-progress` hits it too), that it recurred at 01-01 and 05-01, and that 4 of 6 v0.12.x rows are wrong — two blank-dated and obvious, two wearing a plausible `Complete` with a v0.13.x date. Only the row this phase damaged was repaired (`3f4c4602`); rows 1/2/4 left as user-deferred drift. User decided the defect stays tracked in the spine and is NOT filed upstream. Milestone note: v0.13.x Phase 1 (Interface Enforceability) remains the one outstanding phase. [Corrected at milestone close 2026-08-12: this was FALSE when written — Phase 1 completed 2026-08-04 and was verified `passed`. It was read off the ROADMAP Progress table while that table was mid-corruption by the `cvvrwjbsnz` wrong-milestone write, which is exactly the failure mode of trusting a tracking artifact over the phase's own VERIFICATION.md. Left in place rather than silently rewritten.] Prior: 2026-08-11 after Phase 03.1 (Merge Supersession) — validated REQ-merge-supersession + REQ-merge-atomicity + REQ-merge-idempotency: `supersede_memory` now accepts a SET of `supersedes` targets, so consolidating duplicates links every predecessor to one surviving record with no `delete_memory` anywhere in the merge path. A wave-1 blocking decision gate (`03.1-00`) ruled four discretionary choices BEFORE any code implemented them — moved to the front after cross-AI review found the approval checkpoint originally sat in wave 4, after the cap and class order had already shipped AND been published (a gate that runs after publication is a review, not an approval). Ruling `no-cap-otherwise-defaults`: PD-07 target-set cap DROPPED (no `maxSupersedeTargets`, no `maxItems` — only `minItems: 1`; the input stays unbounded and the reconciliation pass is the single mechanism covering both partial-failure classes, since D-15 already declined the 32-cap and any cap above 32 closes nothing; the DoS residual was quantified at two sequential round trips + one held lock per target and accepted); PD-06 `collapse-ambiguous` ADOPTED (resolution-not-found, resolution-ambiguous, fetch-not-found and not-owned all collapse into one not-found sentinel BEHIND the ownership stage, closing an existence oracle — `ResolvePointID` is owner-agnostic and scrolls the whole collection); PD-01 `dedupe-silently` over RESOLVED UUIDs (mandatory, not stylistic — the locker is a non-reentrant `sync.Mutex`, so locking one target twice self-deadlocks with no panic and no race-detector hit); PD-02 `allow-heterogeneous`. Delivered across 5 waves: scalar→set promotion end to end with a Kind-branching tolerant decoder closing the `GetStringValue()`-returns-"" seam (`supersede_memory` is MCP-only — no Connect/proto RPC, so field-number reasoning never applied); a reconciliation pass for Qdrant's multi-ID `SetPayload`, which is NOT all-or-nothing at the pinned v1.18.2/v1.18.3 (it chunks by 32 and mutates within a chunk BEFORE raising `PointIdError`, falsifying D-06 — memory `mc1d0jmh69`); a four-class staged rejection preflight naming every offender of one class; `idempotency_key` with a fingerprint over content AND the canonicalized target set, replay checked before preflight; and a docs/CLAUDE.md/skill alignment bound by `TestSupersedeDocsMatchShippedContract` to the production `renderTargetRejection` output rather than a re-typed format string (the mirror-by-hand shape that already leaves `guides/cli.md` able to go stale with tests green). Post-execution code review found 1 Critical + 2 Warnings, all fixed and re-verified: CR-01 (`9ef8148f`) was a THIRD member of this repo's lock-coverage family (after `86q25vq6jf`, `m43h2yt97m`) — `lockTargets` locked only the targets, never the survivor's own id, which on the keyed path is the deterministic `idempotencyPointID(owner,scope,key)`; two concurrent keyed calls with the same key and DISJOINT target sets therefore contended on nothing, both passed preflight, and both Upserted the same point id, leaving the loser's targets `superseded_by`-pointing at a survivor whose `Supersedes` omitted them, with NO error and nothing logged. The fix locks the UNION (dedupe spans it, so a survivor id coinciding with a target never self-deadlocks) while keeping the back-stamp STAMP set targets-only, AND re-decides the fingerprint UNDER the lock — locking alone was insufficient because `checkIdempotentMergeReplay`'s decision is made before any lock is held (memory `fvf6dvjxhn`). Verified 11/11 must-haves with 543 `--- PASS` under `-race` against a real dockerized Qdrant. Tech debt carried: two pre-existing `gofmt -l` drifts (`cmd/engram/client_list_test.go` from Phase 01-02, `cmd/engram/migrate_test.go` from Phase 03-03) will fail CI's `gofmt` job, and `task fmt` was observed rewriting `''` into `”` inside a Go comment. Prior: 2026-08-03 — OPENED milestone v0.13.x — Curation & Self-Evidence (`/gsd-new-milestone --reset-phase-numbers`, per rule `rvmts69cz1`): promoted both backlog items — 999.1 (spine review/consolidate/verify/purge/archive) and 999.2 (correct-by-reading CLI + MCP surface audit) — plus the related open issues #453 (documented flag exclusivity never enforced; cobra's `MarkFlagsMutuallyExclusive` used zero times in the repo), #452 (no CLI request timeout on any RPC path), #467 (filed this session: one binary with two exit-code taxonomies — D-09 deliberately left the six operator-command sites on exit 1 while the client lane uses D-17's 0/2/4/5, and no surface says which governs which), and #355 (drifted `tools.go` citation anchors — itself a fixture for 999.1's verify step). 999.1's open shape question resolved at scoping: BOTH a CLI and a skill, split by predicate type — structural predicates to `engram spine-review`, semantic judgments ("is this still true", "are these the same fact") to an agent procedure, mirroring v0.12.x Phase 6's routing. Nyquist validation reconciliation pulled from deferred into scope (memory `bsbsvn4hbc`: a stale `-run` matching nothing exits 0 and reports a false green forever). Version validated per rule `0v4249kc9d` before any write: tag `v0.12.0` + `bump-minor-pre-major: true` / `bump-patch-for-minor-pre-major: false` → release-please cuts `v0.13.0`; no open release PR; one worktree on `feat/v0.13`; no competing milestone branch or workstream. Tracker hygiene at open: #360 and #351 verified delivered in v0.12.x against code (not roadmap prose) and closed with evidence. Prior: 2026-08-02 — CLOSED milestone v0.12.x — Headless Reach & Diagnosability (`/gsd-complete-milestone`): 7 phases (1–7, the first milestone on restarted per-milestone phase numbering), 28 plans, 68 tasks, 21/21 requirements verified, audit `tech_debt` (5/5 cross-phase integration seams, 2/2 E2E flows re-traced, 0 blockers); archived to `milestones/v0.12.x-*`. Delivered the headless reach half — one composed verifier chain built exactly once and injected into both lanes so they provably cannot drift (D-06), enforced token expiry with a byte-identical 401 body, a CSRF exemption re-keyed off a compiler-enforced server-set lane stamp that closes the milestone's #1 risk (a cookie caller opting itself out of CSRF, D-08/D-09), an opt-in fail-closed `connect.headless` mount (D-10/D-11), and `engram search|store|list` over the generated Connect stubs — and the diagnosability half: one `argError` envelope carrying `field=<name> hint=<code>` across ~30 validation sites on both wires with the failure CLASS (not a hand-wrap) selecting the Connect code, bounded authz decision logging at `internal/store`'s two chokepoints with `Decision.diag` kept unexported so a future `cedar.Diagnostic` structurally cannot leak, embedder error bodies surfaced and drained, and a new docs-site `reference/errors.md`. `cross_spine` landed on MCP, then the Connect wire via six additive protobuf fields, then — in Phase 7, closing a seam the milestone audit itself found — the CLI. Phase 6 investigated why `store_rule` never fired, found the propose-a-rule permission buried inside a prohibition, and validated the fix behaviorally: a cold-read agent with zero phase context unprompted named the trigger, proposed via the corrected protocol, and stopped at consent, with the user-blessed gate provably intact. Zero new Go dependencies across all seven phases. Two decisions recorded at close: GSD never creates git tags here (`git.create_tag: false` — release-please owns the namespace, matching the v0.9.x–v0.11.x precedent of zero milestone tags), and the long-standing STATE.md claim of a repo-local `commit.gpgsign=false` was verified FALSE and dropped after surviving three milestone closes. Tech debt carried: no phase has a reconciled Nyquist `VALIDATION.md` (six at `status: draft`, phase 2 has none), and the two-tier CLI error model has no REQ or decision ID recording its exemption from the field+hint envelope. No successor milestone scoped — run `/gsd-new-milestone`, or promote backlog 999.1/999.2 with `/gsd-review-backlog`. Prior: 2026-07-29 — OPENED milestone v0.12.x — Headless Reach & Diagnosability (`/gsd-new-milestone`): scoped the headless client lane (#343 — user chose the Connect route over MCP-over-HTTP, which makes bearer identity on the Connect lane + an opt-in headless mount a prerequisite rather than a flag, since `connectapi.go:362` returns early when `resolve == nil` and Connect is therefore absent entirely on a headless deployment today), cross-spine memory recall as boolean parity with `search_discovery` (#344), a rule-capture investigation-then-fix (#351 — deliverable scope decided by evidence, not assumed to be skill wording), authz diagnosability (#394), failure legibility (#347 embed provider error bodies, #360 misleading over-long-`summary` validation error), and a completion tail (#350 per-lane API key, #345 reindex-resume tag staleness, #356 UI/proto codegen drift). Milestone #1 risk recorded: the Connect CSRF exemption must key on subject-resolution provenance (bearer vs sealed cookie), never on `X-CSRF-Token` header absence — otherwise a cookie caller opts itself out of CSRF; wants the v0.11.x fail-closed-as-first-test treatment. Posture note recorded: mounting Connect headless can give a deployment a surface it does not have today, so it is opt-in only. Prior: 2026-07-26 after CLOSING milestone v0.11.x — Capture & Service Identity (`/gsd-complete-milestone`): 5 phases (22–26), 19 plans, 46 tasks, 11/11 requirements verified, audit PASSED (6/6 cross-phase integration seams, 2/2 E2E flows, 0 blockers); archived to `milestones/v0.11.x-*`. Closed with Phase 26 — validated REQ-memory-citations + REQ-category-filter + REQ-chat-base-url: `payload()`'s discovery-only gate split so `citations` writes for ANY category while `kind` stays discovery-exclusive (D-01), `Citations` declared once on shared `storeArgs` and inherited by schedule/supersede via Go embedding (D-04), compact-recall omission needing an explicit Connect-side `pb.Citations = nil` that closed an information-disclosure gap MCP never had (D-07); `store.SearchOptions` replacing the positional filter tail across ~25 call sites (D-09) with a plural `categories` OR-filter as a hard Qdrant pre-filter and additive proto field 8 (D-10, one-way, user-approved at a blocking checkpoint) carrying no `buf.validate` allowlist since `discovery`/`rule` are legitimate filter values (D-11); and `ENGRAM_OPENAI_CHAT_BASE_URL` with the provider-shape join hoisted into stdlib-only `internal/openaiurl` shared by embed+summarize (D-13/D-14), fixing a live doubled-`/v1` bug Phase 13 had only ever fixed on the embedder lane. Code review caught 1 Critical — the new `Citations` field was excluded from `contentFingerprint`'s explicit (non-reflective) field list, so a keyed replay changing only citations hashed identically and silently discarded the caller's value (fixed `c222c783`; the recurring lesson is that any new client-authored `storeArgs` field MUST be added to the fingerprint in the same change). Milestone tech debt carried forward: Nyquist `VALIDATION.md` left at `status: draft` on phases 24 and 26, and ADR `engram-cdr1` does not name the scoped-vs-blanket forbid distinction it adopted. No successor milestone scoped — run `/gsd-new-milestone`. Prior: 2026-07-19 after Phase 25 (Supersession with History) — validated REQ-supersession-links: a dedicated `supersede_memory` MCP verb (DEC-90w; `supersedeArgs` embeds `storeArgs`) stores a new/correcting record and additively back-stamps the target's `superseded_by` link via a single-key `SetPayload` merge — the `SetVisibility` shape, vector-preserving, NOT a full re-Upsert (D-01, avoids the CR-01 lost-write hazard). Superseded records are soft-hidden at the recall gate (`superseded_by IS EMPTY` at BOTH Search and List call sites) but stay fetchable by id via ungated `get_memory` (D-09, SC2); the op routes through the ownership write gate (`getWritable`/`ActionWrite`, 404-indistinguishable for a not-owned target, SC3) and rejects an already-superseded target (`store.ErrAlreadySuperseded` → Connect `CodeFailedPrecondition`) so a single live head holds and cycles/self-supersession/auto-supersede are impossible (SC4). Correction is explicit and history-preserving, never delete/overwrite (SC1). Zero new dependency; verified 9/9 must-haves + 4/4 SCs live under `-race` vs real Qdrant (testcontainers), lint/vet/license clean, STRIDE threats mitigated in code (a recurring stale-LSP "undefined symbol" false-positive on the new `Store.Supersede`/`spyStore.Supersede` symbols was confirmed benign via `go build`/`go vet`/`go test`). Prior: 2026-07-18 after Phase 24 (Idempotent Capture) — validated REQ-idempotent-capture: optional `idempotency_key` gives strict replay-safety (reject-not-upsert) via a deterministic UUIDv5 point-ID over injective `(owner,scope,key)` (owner-in-hash → no cross-owner poisoning), a payload-only wire-invisible `IdempotencyFingerprint` compared before the embedder call, and `store.ErrIdempotencyConflict`→Connect `AlreadyExists` on mismatch; keyless writes byte-for-byte unchanged (D-01), no new dep/collection/index/config-knob, Connect read lane untouched. Verified 5/5 SCs under `-race` vs real Qdrant; deep code review + `--auto` fix loop closed 1 Critical (concurrent-keyed `short_id` read-back) + 3 Warning + 2 Info to a clean re-review; threat-secure 4/4 (`24-SECURITY.md`). DEC highlights: D-07/D-08 locked deterministic-ID input kept unchanged (cross-tool key-namespace sharing + anonymous-bucket collision documented, not "fixed"). Prior: 2026-07-17 after Phase 23 (Service Auth Chain & Tenancy Isolation) — validated REQ-service-auth-chain + REQ-static-token-auth + REQ-service-owner-failclosed + REQ-service-principal-isolation: pluggable `auth.ChainVerifier` (OIDC user → client-credentials second-Verifier → static-token) wired at the single `withAuth` call site with a structural JWT-vs-opaque discriminator + deny-by-default; the milestone #1 risk proven fail-closed as the FIRST test (`TestFailClosedRejectsEmptyOwner`); static tokens `ConstantTimeCompare`d to per-token namespaced owners; service-principal private isolation proven against Phase-22 store filters (zero new store code); `shared`-cross-tenant kept global as an explicit written+tested decision (ADR `engram-svct`, per-tenant scoping deferred to full ABAC); verified 12/12 must-haves; DEC-12c (no 3rd Subject variant) + DEC-g37x (`namespacedOwner` reuse) + DEC-cgb honored; code review caught+fixed 1 CRITICAL (static-token token↔owner map-orientation inversion, CR-01, broke the lane + could leak the raw token as owner) + 2 warnings + a missing-Expiration bug, all re-verified green. Prior: 2026-07-17 after Phase 22 (Cedar Authz Foundation & Store Enforcement) — validated REQ-cedar-pdp-foundation + REQ-cedar-store-enforcement: `internal/authz` cedar-go v1.8.0 PDP (4-policy embedded corpus incl. scoped empty-owner forbid, policy-text regression suite) consulted by `internal/store`'s bulk filter-builders (`DecideBucket`, O(buckets) never O(records)) and id-addressed gates (`DecideRecord`, Deny→uniform ErrNotFound per DEC-xa6); behavior-preserving (isolation/sharing suite byte-for-byte); ADR `engram-cdr1` refines LOCKED DEC-cgb; verified 5/5 success criteria; code review 0 blockers (2 advisory warnings: DeleteAll not yet PDP-backed, getWritable Deny-mapping untested). Prior: 2026-07-16 — OPENED milestone v0.11.x — Capture & Service Identity (`/gsd-new-milestone`): scoped capture ergonomics & correctness (#340 idempotency/upsert, #341 provenance/citations, #342 supersession links, #374 category filter over MCP) + service-principal access & isolation (#362 pluggable service auth — OIDC client-credentials + static-token fallback, #373 tenancy isolation) + per-lane embedder/chat base URLs (#350); milestone name "Capture & Service Identity", pluggable-auth leaning, research-first. Prior: 2026-07-16 — CLOSED milestone v0.10.x — Hardening & Write Lane (`/gsd-complete-milestone`): 9 phases (13–21) shipped, 19/20 requirements verified (REQ-ci-renovate-spa-drift's live self-heal observation deferred post-merge → #369), audit `tech_debt` (9/9 Nyquist, 0 blockers), archived to `milestones/v0.10.x-*`. Prior: 2026-07-13 after Phase 18 (Stateless Session Rotation) — validated REQ-session-rotation: stateless sliding-expiry re-seal (`webauth.Handler.Reseal` + `newConnectResealInterceptor`, innermost, read+write, absolute `now+TTL`, dual-cookie refresh, zero server-side state, no new `ENGRAM_` var), hard-expiry stays byte-for-byte strict (skew threshold-only, SC4), 50-goroutine `-race` forward-monotonic proof (SC3), and hand-authored ADR `engram-slr8` naming the real `ENGRAM_UI_COOKIE_KEY` kill-switch (not the phantom `ENGRAM_SESSION_KEY`); verified 5/5 must-haves (#323); mandatory `/gsd-secure-phase 18` pending. Prior: 2026-07-13 after Phase 17 (Wired Write Handlers — Full CRUD + Schedule) — validated REQ-connect-write-authz-parity: all six Connect write RPCs are thin adapters delegating to the shared `deps.*` MCP business-logic layer via an explicit `caller` seam + single `protoconv` conversion + single `connectError` mapper (with `context.Canceled`/`DeadlineExceeded` arms), read handlers rewired onto the typed single-path core (D-07); proven by per-RPC MCP↔Connect `TestWriteParity` (identical rejections + AST delegation sub-test) and `TestCrossOwnerRewrap` (no existence leak, DEC-xa6), the `NO_SIDE_EFFECTS` idempotency ban re-asserted in CI, and a fail-closed `requireQdrant` gate pinned to Qdrant v1.18.2; verified 5/5 must-haves (#322). Prior: 2026-07-11 after Phase 16 (CSRF Interceptor) — validated REQ-connect-csrf: the write lane's two-layer transport CSRF defense (Go 1.26 `net/http.CrossOriginProtection` whole-server wrap + Connect-shaped `permission_denied` deny handler; double-submit HMAC-over-Owner token interceptor placed `subject → CSRF → validate`, write-only allowlist on generated Procedure constants, non-HttpOnly `engram_csrf` cookie minted in `Callback`) verified 9/9 must-haves; flagged for `/gsd-secure-phase` (#322). Prior: 2026-07-11 after Phase 15 (Additive Proto + Stub Write Handlers) — validated REQ-connect-write-rpcs: the six additive write RPCs now exist in the Connect wire contract (buf.validate + hand-rolled protovalidate interceptor, auth 401 → validate 400), additive-only, provably GET-unreachable via `UnimplementedEngramServiceHandler` stubs + the `NO_SIDE_EFFECTS` idempotency-ban gate + descriptor/negative-matrix tests; handler bodies deferred to Phases 16–19 (#322). Prior: 2026-07-11 after Phase 14 (Embedder Model Options & Eval) — validated REQ-embed-gemini-direct / REQ-embed-prod-parity-eval / REQ-embed-model-docs via live differ + qwen3@4096 recall@8=1.00 evidence (closes #261/#334/#331). Prior: 2026-07-11 after Phase 13 (Embedder Reliability Foundation) — shipped REQ-embed-timeout / REQ-embed-baseurl-join / REQ-embed-config-identity. Prior: 2026-07-10 opened milestone v0.10.x — Hardening & Write Lane (`/gsd-new-milestone`); 2026-07-10 shipped + archived v0.9.x — Recall Quality (PR #336/#338); 2026-07-08 folded 31 companion ADRs + 24 plans into the baseline; 2026-07-07 retrospective baseline ingest (v0.8.x shipped).*
