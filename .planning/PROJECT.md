...
## Current State: v0.12.x — Headless Reach & Diagnosability ✅ SHIPPED (2026-08-02)
...
## Current Milestone: v0.13.x — Curation & Self-Evidence

**Goal:** Make the memory spine maintainable without hand-curation, and make every command, flag,
and tool argument correct-by-reading — closing the two classes v0.12.x left to human diligence.

**Target features:**

- **Spine curation, split by predicate type** (promoted from backlog 999.1) — an `engram
  spine-review` CLI resolving **structural** predicates (drifted `file:line` citations, orphaned
  records, lapsed windows, extract-before-delete ordering), plus a curation **skill** for the
  **semantic** judgments a CLI cannot make ("is this record still true", "are these two the same
  fact"). This is the split v0.12.x Phase 6 already made when it routed its backfill sweep to an
  agent procedure rather than a command. Rule `7smp8vy9hr` mandates the curation pass today but
  nothing enforces its ordering, detects when it is overdue, or verifies the extract happened
  before the delete.
- **Self-evident surface audit** (promoted from backlog 999.2) — every `cmd/engram/*` flag help
  string and `Short`/`Use`, the v0.12.x Phase 2 D-15 self-describe JSON catalog, and the
  `internal/server` MCP tool + argument docs, against the correct-by-reading principle
  (`4aksmneehh`, D-00). Server-side conditional-requirement rules (`effectiveSearchScope` and
  siblings) must be stated wherever the argument is advertised.
- **Documented constraints made enforceable** (#453) — `client_list.go` declares
  `--offset`/`--cursor-mode`/`--page-token` mutually exclusive in help text and validates none of
  it. Cobra ships `MarkFlagsMutuallyExclusive` and this repo uses it zero times, so the fix is
  systematic rather than a third hand-rolled guard alongside `client_common.go:236` and
  `migrate.go:85`.
- **CLI request timeout** (#452) — `newHTTPClient` sets no `Timeout` and no RPC path applies a
  context deadline, so a cron loop can hang forever. Needs a `--timeout` flag with a finite
  default and an exit-code mapping.
- **One exit-code taxonomy, or a documented boundary** (#467) — `engram search` answers a
  contradictory flag pair with exit 2; `engram migrate-remap-owner` answers the same class with
  exit 1, per D-09's deliberate backward-compatibility carve-out. Six operator-command sites are
  affected and no surface states which taxonomy governs which command.
- **Nyquist validation reconciliation** — no v0.12.x phase has a reconciled `VALIDATION.md` (six
  at `status: draft`, Phase 2 has none). Not inert debt: memory `bsbsvn4hbc` records that the
  `-run` commands written at plan time routinely match nothing, and `go test -run` matching
  nothing exits 0, so a stale row reports a false green forever.
- **Drifted doc anchors** (#355) — stale `tools.go` line-number citations in the retrievaleval
  test comments and a dangling docs cross-ref. Also a live fixture for the spine-review verifier:
  #355 *is* the drifted-citation failure class 999.1's verify step exists to detect.
...
- **Two-tier CLI error model** — the client-side usage-error/exit-2 lane has no REQ or decision ID
  recording its exemption from the field+hint envelope. Adjacent to #467 and may be absorbed by it.
...
None — v0.12.x closed 2026-08-02 and no successor milestone is scoped. Run `/gsd-new-milestone`,
...
- [ ] **Phase 13–15 review follow-ups** — #346 (deliberate non-fix; Phase 26's `TestJoin` pins the behavior — consider closing with that rationale), #353/#354 (eval-differ defects), #355, #357, #358
- [ ] **Two-tier CLI error model undocumented** (v0.12.x) — Phase 7's client-side `validateScopeCrossSpine` returns a plain usage error with exit 2 rather than Phase 4's `field=<f> hint=<code>` envelope. Intentional and correctly scoped, but no REQ or decision ID records the exemption; write one so the split is deliberate rather than incidental.
- [ ] **Nyquist `VALIDATION.md` reconciliation** (v0.12.x) — no phase has a reconciled file: six sit at `status: draft` (seeded by plan-phase, never reconciled) and phase 2 has none. Note the recorded `-run` commands are plan-time and routinely stale, so re-resolve each against `go test -list` rather than trusting a green row (`bsbsvn4hbc`).
