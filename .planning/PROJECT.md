...
Not yet scoped — run `/gsd-new-milestone` to define the next milestone. Carried into that
conversation:
...
- **Versioned payload-migration mechanism** — there is no stored schema/payload version anywhere;
  every evolution to date shipped as its own one-shot operator command with a near-identical shape
...
None — v0.13.x closed 2026-08-12 and no successor milestone is scoped. Run `/gsd-new-milestone`,
or promote a backlog phase with `/gsd-review-backlog`.
