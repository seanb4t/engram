...
## Current Milestone: v0.12.x Headless Reach & Diagnosability

**Goal:** Make engram usable by agents that aren't a top-level MCP client, and make what the
server decides and rejects legible to the humans and agents on the other end.

**Target features:**

*Reach — the headless client lane (#343, the milestone's spine)*
- Bearer-token identity on the Connect lane, reusing the shipped `auth.ChainVerifier`
  (OIDC user → client-credentials → static token)
- Connect mountable **headless** — decoupled from the UI-enabled gate, explicitly opt-in
- CSRF exemption keyed on resolution *provenance* (bearer vs sealed cookie), cookie lane untouched
- `engram search | store | list` client subcommands over the typed Connect stubs

*Reach — recall & capture semantics*
- `cross_spine=true` on `search_memory`, mirroring `SearchDiscoveryArgs.CrossSpine`; `scope`
  becomes optional, authz gates unchanged (#344)
- Investigate why rule capture never fires, then fix the actual break point (#351)

*Diagnosability*
- Wire `authz.Decision.diag` to debug logging / span events under DEC-wot's owner-only PII
  rules — it is computed on every decision and has zero readers (#394)
- Surface a bounded prefix of the provider's error body on non-2xx embeds, then drain for
  keep-alive (#347)
- Stop reporting `missing properties: ["content"]` when the real fault is an over-long
  `summary` (#360)

*Completion tail*
- Per-lane **API key** — the base-URL half shipped in Phase 26; this closes #350
- `reindex --resume` must include `tags` in the equality key; a tag-only edit currently leaves
  a stale vector (#345)
- Sync `ui/src/lib/gen/engram_pb.ts` with proto and extend the CI drift check beyond `gen/` (#356)

**Milestone #1 risk — CSRF exemption provenance.** `newConnectCSRFInterceptor` is write-gated and
checks a double-submit HMAC bound to `Subject.Owner`. A bearer caller carries no ambient cookie, so
CSRF does not apply to it — but the exemption MUST key on *how the subject was resolved*, never on
the presence or absence of an `X-CSRF-Token` header. Keyed on header absence, a cookie caller can
opt itself out of CSRF by omitting the header. Structurally identical to v0.11.x's `owner==""` risk
and wants the same treatment: proven fail-closed as the phase's FIRST test.

**Posture note.** `mountConnect` today returns immediately when `resolve == nil` (R1 — UI disabled
means Connect is not mounted *at all*). Mounting headless means a deployment with no Connect
surface today could gain one: opt-in only, never a default flip.

**Deliberately still open (not this milestone):**
...
**v0.12.x — Headless Reach & Diagnosability** (opened 2026-07-29). Requirement IDs and their
phase mapping are defined in `.planning/REQUIREMENTS.md` and `.planning/ROADMAP.md`; the scoped
themes are:

- **Headless client lane** — Connect bearer identity, headless mount, provenance-keyed CSRF
  exemption, and `engram search|store|list` subcommands (#343)
- **Cross-spine memory recall** — `cross_spine` parity with `search_discovery` (#344)
- **Rule capture** — investigate the break, then fix it (#351)
- **Authz diagnosability** — `Decision.diag` reaches a reader (#394)
- **Failure legibility** — embed provider error bodies (#347), honest argument-validation
  errors (#360)
- **Per-lane provider config** — distinct API key, closing #350
- **Reindex resume correctness** — tag-aware equality key (#345)
- **Codegen drift** — UI TS types synced, CI check extended beyond `gen/` (#356)
