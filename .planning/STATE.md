...
status: in_progress
stopped_at: "v0.12.x Phase 7 (CLI Cross-Spine Wiring) COMPLETE — 3/3 plans, all 3 waves landed; 07-VERIFICATION.md status=passed, 10/10 decisions (D-00..D-09) verified against the codebase. Phase base for all diff gates was b4544d47; internal/ containment held at exactly internal/server/tools.go and go.mod/go.sum drift was zero. This closes the last open phase of v0.12.x — all 7 phases complete. Next: the milestone lifecycle (/gsd-ship or /gsd-complete-milestone), plus the still-open non-blocking item carried from Phase 6 (the live rule backfill sweep with Sean, 06-DEMONSTRATION.md result section)."
last_updated: "2026-08-02T15:50:00.000Z"
progress:
  total_phases: 7
  completed_phases: 7
  total_plans: 28
...
**Current focus:** v0.12.x Phase 7 — CLI Cross-Spine Wiring — ✅ COMPLETE, 3/3 plans, verification PASSED 10/10 decisions. Wired the `cross_spine` capability (shipped on the Connect API in Phase 3, unreachable from the CLI until now) through to `engram search` and `engram list`: one shared `validateScopeCrossSpine` guard fires before any dialing, `renderCoverageFooter` prints the D-05 coverage line, and `EffectiveSearchScope` — the phase's sole authorized `internal/` edit — pins the client guard against the server's own rule at compile time. 07-03 closed D-00's correct-by-reading bar: the self-describe catalog absorbed the flag with zero catalog production code (verified empirically, not assumed), and `--scope`/`--cross-spine` name each other in live `--help` on both commands. **This was v0.12.x's last open phase — all 7 phases COMPLETE.** Next: the milestone lifecycle (`/gsd-ship` or `/gsd-complete-milestone`), plus Phase 6's still-open non-blocking item (the live rule backfill sweep with Sean). Prior phase for reference: v0.12.x Phase 6 — Rule Capture — COMPLETE, 3/3 plans, all 3 requirements. Plan 06-01 de-buried the propose-a-rule permission (`### Proposing a rule`: two observable triggers, an inline propose-then-consent protocol, a `rule-declined` decline record) and closed `REQ-rule-capture-investigation` by citation; its cold-read checkpoint (reassigned per D-14) PASSED (`06-COLD-READ.md`). Plan 06-02 added `### Rule hygiene` (duplicates/contradictions/rot, a code-verified correction table, D-10's user-blessed deletion gate, D-11/D-15-priced full-text check) and `### One-time rule backfill sweep` (D-12's procedure), and mirrored both into the tool reference, closing `REQ-rule-curation-hygiene`. Plan 06-03 restored the milestone-completion cadence clause (rule `7smp8vy9hr`) and closed `REQ-rule-capture-intervention` by citation to `06-COLD-READ.md`'s PASS — the live backfill sweep against the three D-03 candidates was reassigned to the orchestrator (no MCP access in the executing environment) and scaffolded in `06-DEMONSTRATION.md`, pending but non-blocking. **v0.12.x Phase 6 is the milestone's last phase — all 6 phases of v0.12.x are now COMPLETE.** Next: the orchestrator runs the milestone lifecycle (live sweep with Sean, then ship/complete-milestone).
...
Phase: v0.12.x Phase 7 (CLI Cross-Spine Wiring) — ✅ COMPLETE 2026-08-02 (3/3 plans, all 3 waves
landed). Verification PASSED, 10/10 decisions (D-00..D-09). `--cross-spine` now reaches `engram
search` and `engram list` through ONE shared `validateScopeCrossSpine` guard
(`cmd/engram/client_common.go:234`, called from `client_search.go:40` and `client_list.go:38`)
that fires before any dialing; `renderCoverageFooter` prints the D-05 coverage line; the guard is
pinned against `internal/server.EffectiveSearchScope` at compile time. `internal/` containment
held at exactly one file vs base `b4544d47`, zero new dependencies.
Commits: ab133d77, 327fa9d6, 119cb2f8, ec1fc8f5 (07-01); b582e82a, 7821500f, c5c568fc (07-02);
d3f47669, 6a32bb01, d6f00970, 596abc28 (07-03).
Executors ran sequentially on the main working tree — `worktree.base-check` auto-degraded
isolation to `none` (#683) because HEAD had diverged from `origin/HEAD`; no parallelism was lost
since every wave held exactly one plan.
**This was the last open phase of v0.12.x — all 7 phases are now COMPLETE.**
